...
def get_compressed_file(csv_path):
        os.chdir(csv_path)
        os.system("rm *.tar.gz")
        gz_file = 'HC_CHHATTISGARH_%s.tar.gz' % str(datetime.now().date().strftime('%d%m%Y'))
...
def send_mail(csv_path):
        try:
            ...
            recievers_list = ['akram@headrun.net']
            ...
            msg['Subject'] = 'HC Chhattisgarh POC: Data on %s' % (
                str(datetime.now().date()))
            mas = '<h3>Hi Team,</h3>'
            mas += '<p>Please find the High Court of Chhattisgarh data in the below attachment</p>'
...
class HCChattisgharfile(object):

    def __init__(self):
        self.main_path = os.path.dirname(os.getcwd())
        self.csv_path = os.path.join(self.main_path, 'spiders/csv_files', datetime.now().strftime('%Y-%m-%d'))
    ...
    def main(self):
        ...
        if os.path.isfile(file_name):
            ...
            for row in rows:
                row = row.replace('\r\n', '').strip()
                word, year = row.split('|')
                cmd = "scrapy runspider hc_chattisghar_browse.py -a word='%s' -a year='%s' -a path='%s'" %(word.strip(), year.strip(), self.csv_path)
                os.system(cmd)
